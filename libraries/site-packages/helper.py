...
def get_seed(random_seed=''):
    max_seed = 1000000
    if random_seed == '':
        return int(random.random(0, max_seed))
    else:
        try:
...
def set_seed(random_seed=0):
    random_seed = int(random_seed)
    random.seed(random_seed) # This only applys to the Python random functions
...
def random_list_value(val_list):
    ...
    # Could be replaced by https://docs.python.org/3/library/random.html#random.choice ?
    index = int(random.random(0, len(val_list)))
...
def random_centered(value_og, offset=5):
    ...
    value = random.random(value_og-offset, value_og+offset)
    return value


def random_gaussian_limit(min_val, max_val):
    '''Same as built-in randomGaussian but truncated to within a range'''
    # Should this be https://docs.python.org/3/library/random.html#random.gauss ?
...
def circle_points(origin_x, origin_y, r=50, a=0):
    ...
    x = origin_x + (r * math.cos(a))
    y = origin_y + (r * math.sin(a))
    return x, y
